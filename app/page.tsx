...
export default function MainMenu() {
  ...
  const highScore = SaveManager.highScore;
  const totalRuns = SaveManager.totalRuns;
    ...
    <div className={`w-screen h-screen bg-black flex justify-end items-center overflow-hidden select-none relative transition-opacity duration-700 ${isExiting ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className="z-10 w-full md:w-1/2 flex flex-col justify-center items-start px-20">
        ...
        <div className="flex flex-col gap-6 text-3xl font-bold text-gray-500">
          ...
          <a href="/settings" className="menu-item cursor-pointer text-gray-500 hover:text-blue-400 flex items-center gap-4">
            <span className="text-blue-600 text-xl">►</span> SETTINGS
          </a>
        ...
        {/* Stats */}
        {totalRuns > 0 && (
          <div className="mt-8 text-gray-700 text-xs space-y-1">
            <p>HIGH SCORE: {highScore} | RUNS: {totalRuns}</p>
          </div>
        )}

        <p className="absolute bottom-10 left-20 text-gray-800 text-xs tracking-widest">BUILD v0.2.0 BETA</p>
      </div>
    </div>
  );
}
